...
export default function Enrollments() {
  ...
  const handleApprove = async (enrollmentId: string, role: string) => {
    ...
    try {
      ...
      setEnrollments(prev => (prev ?? []).map(e => (e as any).id === enrollmentId ? { ...e, status: 'approved' as const } : e))
    } catch (err) {
  ...
  const handleReject = async (enrollmentId: string) => {
    ...
    try {
      ...
      setEnrollments(prev => (prev ?? []).map(e => (e as any).id === enrollmentId ? { ...e, status: 'rejected' as const, rejection_reason: rejectReason } : e))
      setRejectingId(null)
      setRejectReason('')
    } catch (err) {
    ...
    <div style={{ padding: '100px 24px', maxWidth: 900, margin: '0 auto' }}>
      ...
      {enrollments === null ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {filtered.map((e) => (
            <div key={(e as any).id} style={{ padding: 20, borderRadius: 10, border: '1px solid #E5E7EB', background: 'white' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 16, fontWeight: 600, color: '#1E3A5F' }}>
                      {(e as any).first_name} {(e as any).last_name}
                    </span>
                    <span style={{
                  ...
                  <div style={{ fontSize: 13, color: '#6B7280', marginTop: 4 }}>
                    ...
                    {(e as any).preferred_subjects && (e as any).preferred_subjects.length > 0 && (
                      <span> Subjects: {(e as any).preferred_subjects.join(', ')}</span>
                    )}
                  </div>
                  {(e as any).parent_email && (
                    <div style={{ fontSize: 13, color: '#6B7280', marginTop: 2 }}>
                      Parent: {(e as any).parent_email} {(e as any).parent_phone && `| ${(e as any).parent_phone}`}
                    </div>
                  )}
                  {(e as any).rejection_reason && (
                    <div style={{ fontSize: 13, color: '#DC2626', marginTop: 4 }}>Reason: {(e as any).rejection_reason}</div>
                  )}
                </div>

                {e.status === 'pending' && (
                  <div style={{ display: 'flex', gap: 8 }}>
                    {rejectingId === (e as any).id ? (
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        ...
                        />
                        <button onClick={() => handleReject((e as any).id)} disabled={processing} style={{ padding: '6px 12px', borderRadius: 6, background: '#DC2626', color: 'white', border: 'none', fontSize: 13, cursor: processing ? 'not-allowed' : 'pointer' }}>
                          {processing ? '...' : 'Confirm'}
                        </button>
                      ...
                      <>
                        <button onClick={() => handleApprove((e as any).id, e.role)} disabled={processing} style={{ padding: '6px 16px', borderRadius: 6, background: '#10B981', color: 'white', border: 'none', fontSize: 13, cursor: processing ? 'not-allowed' : 'pointer', fontWeight: 600 }}>
                          {processing ? '...' : 'Approve'}
                        </button>
                        <button onClick={() => setRejectingId((e as any).id)} disabled={processing} style={{ padding: '6px 16px', borderRadius: 6, background: '#FEE2E2', color: '#DC2626', border: '1px solid #FECACA', fontSize: 13, cursor: processing ? 'not-allowed' : 'pointer', fontWeight: 600 }}>
                          Reject
                        </button>
                      </>
                    )}
